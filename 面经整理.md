...
将析构函数设置为私有，写一个共有的destroy函数释放内存。
...
在类中写出new和selete的重载函数，并将其设置为私有即可
...
（1）将代码按照不同的功能模块进行组织，每个模块放在一个文件夹下。主要有lock模块用于控制互斥锁和信号量，http模块负责解析报文，定时器模块，线程池模块，还有主函数
...
- 握手是为了协商连接参数和初始序列号，挥手是为了协商连接终止请求和确认。
...
Timewait 状态的主要目的是确保在连接终止后的一段时间内，处于 Timewait 状态的一方能够接收到可能延迟到达的报文段吗，并防止早期建立的连接的重复报文段干扰后续连接。
...
（2）**如果客户端在完后一个 HTTP 请求后，在一段时间内都没有再发起新的请求，定时器的时间一到，就会触发回调函数来关闭该连接，那么此时服务端上就会出现 TIME_WAIT 状态的连接**。
...
当服务器需要识别客户端时需要结合Cookie, Cookie就是存储由服务器发至客户端并由客户端保存的一段字符串,  为了保持会话，服务器可以在响应客户端请求时将Cookie字符串放在Set-Cookie下，客户机收到Cookie之后保存这段字符串，之后再请求时候带上Cookie就可以被识别。

每次HTTP请求的时候，客户端都会发送相应的Cookie信息到服务端。第一次创建Session的时候，服务端会在HTTP协议中告诉客户端，需要在Cookie里面记录一个Session ID，以后每次请求把这个会话ID发送到服务器，就可以知道用户信息了。如果客户端的浏览器禁用了Cookie，则会使用一种叫做URL重写的技术来进行会话跟踪，即每次HTTP交互，URL后面都会被附加上一个诸如sid=xxxxx这样的参数，服务端据此来识别用户，这样就可以帮用户完成诸如用户名等信息自动填入的操作了
...
在同一个线程中连续lock两次mutex会产生死锁。一般情况下，如果同一个线程先后两次调用 lock，在第二次调⽤用时，由于锁已经被占用，该线程会挂起等待占用锁的线程释放锁，然而锁正是被自己占用着的，该线程又被挂起而没有机会释放锁，因此 就永远处于挂起等待状态了，于是就形成了死锁。
...
（3）class对象中普通函数成员不会影响对象大小，但class中virtual函数会影响类对象的大小。因为虚函数导致对象增加4字节空间, 这是 vptr的缘故。
...
（4）破坏循环等待条件：按照某一规则对资源进行排序，要求每个进程按照相同的顺序请求资源，可避免进程等待循环的发生。
...
（a）**空指针为什么能调用非虚成员函数？**
        首先，调用非虚函数，采用的是静态联编的方式，在编译期间，编译器会根据对象的静态类型来确定调用哪个函数。p的静态类型是A *，p指向的对象类型是p的动态类型，p->fun0();这种成员函数调用，编译器会根据静态类型A *去找到函数A::fun0()的地址。（其实编译器还给函数传了个this指针，后面会讲）
       因此，不管p指向什么，都不会影响p的静态类型，只要p的静态类型确定了，编译器就会根据这个静态类型找到对应的函数地址，就算p指向了NULL，也能找到A::fun0()地址，自然能成功调用了。
（b）**空指针调用的非虚成员函数中，引用了数据成员，为什么运行时会报错？**
        p->fun0();这种成员函数调用，在编译器里面会转变成A::fun0(this);，this指针会当作一个参数传给函数的。也就是说类对象被当作一个参数传给了成员函数，除了多了个this参数之后，成员函数和一般的函数并没有区别。只要函数内部不对这个this指针做取址操作，就不会报错。
...
可以使用双BUFFER及共享指针的机制来实现对同一变量的高效访问，同时又能保证不会出现竞争条件
...
（2）共享指针记录了对变量的引用次数，可以避免指针切换时的**访问丢失**问题

**在指针的切换过程中，会出现如下两个问题：**

假设使用map作为缓冲buffer

（1）由于对主map 的读是多线程的读，会出现多线程同使用主map 共享指针ptr 的情形，而互换指针时，需要对主map 的指针进行写操作，为了避免对 ptr 的读写出现竞争条件，可以使用自旋锁来对ptr 的读写进行同步。使用自旋锁的原因有两个：
...
（1）如果客户端进程崩溃，那么内核会发送四次挥手来断开TCP连接，此时服务端无法响应FIN报文，因此客户端会在超时重传一定次数后断开连接，然后服务端read函数就会返回一个表示连接已关闭的值，通常是-1，并结束阻塞状态
...
如果开启了TCP Keepalive保活机制，那么一定时间后服务端内核就会向应用程序发送错误通知，通常read返回-1，结束阻塞；
